...
export function useAIChatMessages(options = {}) {
	...
	const shouldShowCard = (message) =>
		message.messageType && (isCardMessage(message.messageType) || message.messageType === 'uni_card')

	/**
	 * [UniCard迁移] 是否显示 UniCard 类型消息
	 * 当 messageType 为 'uni_card' 时，也认为应显示卡片
	 * @param {Object} message - 消息对象
	 * @returns {boolean}
	 */
	const shouldShowUniCard = (message) =>
		message.messageType === 'uni_card'

	/**
	 * [UniCard迁移] 增强版 shouldShowCard
	 * 在旧逻辑基础上增加 uni_card 类型支持
	 * @param {Object} message - 消息对象
	 * @returns {boolean}
	 */
	const shouldShowCardEnhanced = (message) =>
		shouldShowCard(message) || shouldShowUniCard(message)
