...
-- 2. 权限表（如果不存在，添加description字段）
ALTER TABLE `t_permission` ADD COLUMN IF NOT EXISTS `description` VARCHAR(200) COMMENT '权限描述' AFTER `icon`;
...
-- 清理旧数据（可选）
-- DELETE FROM `t_admin_role_relation`;
-- DELETE FROM `t_role_permission_relation`;
-- DELETE FROM `t_role` WHERE role_code IN ('SUPER_ADMIN', 'ADMIN', 'AUDITOR');
...
-- 查看角色列表
-- SELECT * FROM t_role ORDER BY sort_order;

-- 查看权限树
-- SELECT * FROM t_permission ORDER BY parent_id, sort_order;

-- 查看超级管理员的权限
-- SELECT p.* FROM t_permission p
-- INNER JOIN t_role_permission_relation rpr ON p.permission_id = rpr.permission_id
-- INNER JOIN t_role r ON rpr.role_id = r.role_id
-- WHERE r.role_code = 'SUPER_ADMIN'
-- ORDER BY p.parent_id, p.sort_order;

-- 查看每个角色的权限数量
-- SELECT r.role_name, r.role_code, COUNT(rpr.permission_id) as permission_count
-- FROM t_role r
-- LEFT JOIN t_role_permission_relation rpr ON r.role_id = rpr.role_id
